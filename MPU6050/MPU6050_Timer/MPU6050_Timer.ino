...
int16_t n_calibrate=500; //Jumlah pengambilan data untuk kalibrasi
int TCAADDR = 0x70; //Alamat i2c mux tca984
...
#define HW_TIMER_INTERVAL_US  10L //Timer Interval hardware

#define TIMER_INTERVAL_10US   100L //interval timer ISR

//Fungsi untuk mengambil data MPU6050 dengan timer
void dataMpu(){
  // Check TCa
  tcaCheck();
...
//  Kalibrasi Sensor
  ...
  ofsetDat2 = calibrate(0x68,3);
  ofsetDat3 = calibrate(0x68,2);
  waktu_lalu=millis();

  delay(100);
  
//  Mulai Timer Interrupt
  if(Timer1.attachInterruptInterval(HW_TIMER_INTERVAL_US, dataMpu)){
    Serial1.println("Start Timer1 Ok, millis() = "+String(waktu_lalu));
    delay(1000);
  }
  else{
    Serial1.println("Tidak bisa set Timer1, ganti frekuensi atau interval lain");
  }

//  Mulai ISR_timer
  ISR_Timer.setInterval(TIMER_INTERVAL_10US, dataMpu);
}
...
void tcaCheck(){
  ...
  int16_t error = Wire.endTransmission();
    ...
    Serial1.println("Cek Koneksi kabel tca984");
    delay(500);
    Wire.beginTransmission(0x70);
...
void setMPU(int address, int busTca){
  tcaselect(busTca); //pilih bus tca
